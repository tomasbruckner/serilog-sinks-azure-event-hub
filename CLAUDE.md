...
Both sinks build their `EventData` through the shared internal `EventDataFactory`: render the `LogEvent` through the `ITextFormatter` to a string, UTF-8 encode it as the `EventData` body, and attach two properties — `Type = "SerilogEvent"` and `Level = <level>`. When the sink is created with `shouldIncludeProperties: true` (opt-in, threaded from the `WriteTo`/`AuditTo` extension parameter), the factory also copies the log event's structured properties onto `EventData.Properties` — without ever overwriting the reserved `Type`/`Level`. A scalar keeps its CLR value only when its type is in `EventDataFactory.AmqpPropertyTypes` (the set Event Hubs can serialize as an AMQP application property); any other scalar (e.g. an enum) and every structured/sequence/dictionary value is rendered to an **invariant-culture** string, so a property value can never make `SendAsync` throw a `SerializationException`.

Both sinks implement `IDisposable`. They dispose the `EventHubProducerClient` **only when the library created it** (the connection-string overloads, tracked by an internal `ownsClient` flag); a caller-supplied client is left for the caller to dispose. Serilog disposes the sinks when the logger is disposed. The `WriteTo`/`AuditTo` extension methods share their default template, client/formatter construction, and argument validation through the internal `EventHubSinkConfigurationHelper`.
